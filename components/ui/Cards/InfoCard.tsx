...
const InfoCard: React.FC<CardProps> = ({
  ...
  return (
    <div
      ...
      data-state="active"
      className="h-auto group relative w-[300px] min-h-[8rem] overflow-hidden rounded-2xl border border-slate-6 p-6 outline-none focus-visible:ring-2 focus-visible:ring-slate-6 data-[state='active']:border-slate-6"
    ...
    >
      <span className="absolute inset-px z-10 grid place-items-center rounded-2xl group-data-[state='active']:bg-black group-data-[state='active']:bg-gradient-to-b group-data-[state='active']:from-grw/[3%] group-data-[state='active']:via-white/[1%] group-data-[state='active']:to-black">
        <div className="flex flex-col items-center justify-center h-full w-full text-center px-4 py-2">
          <div className="flex h-10 w-10 justify-center rounded-xl border-slate-6 bg-gradient-to-bl from-green/[4%] p-2 text-slate-12 group-data-[state='active']:text-green-9">
          ...
          {description && (
            <p className="text-sm leading-[1.6] text-gray-500 mt-1 flex-grow">
              {description}
            </p> // Mostrar la descripción con el color deseado
          )}
        </div>
      </span>
      <span
