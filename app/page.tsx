    ...
    <main className="flex min-h-screen flex-col bg-gray-900 text-white">
            ...
            <nav className="mt-12">
              <ul className="space-y-6 text-center">
                {['Home', 'About', 'Services', 'Contact'].map((item) => (
                  <li key={item}>
                    <a href="#" className="text-3xl font-bold text-white hover:text-purple-400 transition-colors">{item}</a>
                  </li>
                ))}
              </ul>
            </nav>
      ...
      <section className="text-center mt-24 mb-20 px-4">
        ...
        <p className="text-xl text-gray-300 mb-8 max-w-2xl mx-auto">
          We craft stunning websites and powerful applications that drive your business forward
        </p>
      ...
      <section className="w-full max-w-6xl mx-auto my-20 px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            { title: 'Web Development', description: 'Custom websites tailored to your brand and goals.' },
            { title: 'App Development', description: 'Powerful mobile apps for iOS and Android platforms.' },
            { title: 'UI/UX Design', description: 'Intuitive and beautiful interfaces that users love.' }
          ].map((service, index) => (
      ...
      <footer className="w-full bg-gray-800 text-gray-300 py-12 px-4">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {['Web Development', 'App Development', 'UI/UX Design', 'Consulting'].map((item) => (
                <li key={item}><a href="#" className="hover:text-white transition-colors">{item}</a></li>
              ))}
            </ul>
          </div>
