'use client';

import { ArrowRight } from 'lucide-react';
import Image from 'next/image';
import Link from 'next/link';

import { Button } from '../../components/ui/Buttons/v1/button';

function CustomSection({ children, height, gradientColor }) {
  return (
    <div className={`relative ${height}`}>
      <div
        className="absolute inset-0"
        style={{
          background: `linear-gradient(to bottom, transparent 0%, ${gradientColor} 100%)`,
        }}
      ></div>
      <div className="relative z-10">{children}</div>
    </div>
  );
}

export default function AutomotiveLanding() {
  const products = [
    {
      title: 'Premium Performance Parts',
      image: '/placeholder.svg?height=600&width=400',
      color: '#FF3300', // Distinctive red for performance
      description:
        "Elevate your vehicle's performance with our premium aftermarket parts. Engineered for precision and power, our performance upgrades deliver exceptional results for enthusiasts who demand the very best. Each component is meticulously designed to enhance your driving experience while maintaining reliability.",
      link: '/performance',
    },
    {
      title: 'Luxury Interior Collection',
      image: '/placeholder.svg?height=600&width=400',
      color: '#E6C200', // Gold for luxury
      description:
        "Transform your vehicle's cabin with our exclusive luxury interior collection. Featuring premium materials, ergonomic designs, and impeccable craftsmanship, our interior upgrades create a sophisticated driving environment. From custom leather upholstery to advanced infotainment solutions, we offer the perfect balance of comfort and elegance.",
      link: '/interior',
    },
    {
      title: 'Advanced Lighting Systems',
      image: '/placeholder.svg?height=600&width=400',
      color: '#00A3FF', // Blue for technology
      description:
        'Illuminate the road ahead with our cutting-edge automotive lighting systems. Combining striking aesthetics with superior functionality, our lighting solutions enhance visibility while giving your vehicle a distinctive presence. LED technology provides exceptional brightness, energy efficiency, and longevity for both practical and aesthetic applications.',
      link: '/lighting',
    },
  ];

  return (
    <div className="min-h-screen bg-black text-white selection:bg-white selection:text-black">
      {/* Hero Section */}
      <div className="relative min-h-screen flex items-center justify-center pt-16">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-zinc-900 via-black to-black"></div>
        <div className="relative z-10 container px-4 md:px-6">
          <p className="text-sm uppercase tracking-widest text-zinc-400 text-center mb-6 leading-none">
            Precision engineered automotive excellence for the discerning
            enthusiast
          </p>
          <p className="text-sm uppercase tracking-widest text-zinc-400 text-center mb-6">
            discover our premium collection
          </p>
          <h1 className="font-serif text-center text-7xl md:text-8xl lg:text-9xl tracking-tight mt-5">
            Automotive Elite
          </h1>
          <p className="mt-8 text-lg text-zinc-400 max-w-2xl mx-auto text-center leading-relaxed">
            Where performance meets luxury. Elevate your driving experience with
            our curated selection of premium automotive products.
          </p>
          <div className="flex flex-col items-center space-y-2 mt-16">
            <span className="text-2xl animate-pulse duration-700 delay-0">
              ▼
            </span>
            <span className="text-2xl animate-pulse duration-700 delay-150">
              ▼
            </span>
            <span className="text-2xl animate-pulse duration-700 delay-300">
              ▼
            </span>
          </div>
        </div>
      </div>

      {/* Content Section */}
      <div className="container px-4 md:px-6 py-24">
        {products.map((product, index) => (
          <CustomSection
            height="h-px"
            gradientColor={product.color}
            key={index}
          >
            <section
              key={index}
              className="w-full relative rounded-xl bg-[linear-gradient(180deg,_#FFFFFF3D_0%,_#FFFFFF00_54%)] backdrop-filter backdrop-blur-[4px] p-[.0625rem] mt-24"
            >
              <div className="group flex flex-col md:flex-row gap-14 md:gap-24 w-full px-8 md:px-16 rounded-xl pt-8 pb-4 md:pb-6 bg-[linear-gradient(90deg,_#0000_0%,_#0000_100%),_radial-gradient(30%_36%_at_0%_60%,_#FFFFFF0D_0%,_#FFFFFF05_54%,_#FFFFFF00_100%),_linear-gradient(180deg,_#111_0%,_#00000080_100%)]">
                <div className="flex items-center justify-center md:block translate-x-2 -translate-y-16 md:translate-x-5">
                  <Link href={product.link}>
                    <div className="flex items-center justify-center [transform-style:preserve-3d] [perspective:1000px] [perspective-origin:center]">
                      <div
                        className="relative [box-shadow:20px_20px_20px_rgba(0,_0,_0,_0.2)] [transform-style:preserve-3d] [transform:rotateY(30deg)_rotateX(0deg)] transition-transform ease-in-out duration-500 group-hover:[transform:rotateY(0deg)] group-hover:[box-shadow:0_20px_20px_rgba(0,_0,_0,_0.2)]"
                        style={{ width: '214px', height: '314px' }}
                      >
                        <div className="content-[''] absolute w-[44px] h-full origin-left bg-center bg-[#2a2a2a] [transform:rotateY(90deg)] bg-[linear-gradient(180deg,_#2a2a2a,_#2a2a2a_50%,_#000000_100%)] [box-shadow:-1px_-1px_4px_3px_rgba(0,_0,_0,_0.25)_inset,_1px_1px_4px_0px_rgba(255,_255,_255,_0.1)_inset,_8px_-8px_16px_-8px_rgba(0,_0,_0,_0.08)_inset,_16px_-16px_16px_-16px_rgba(0,_0,_0,_0.25)_inset]"></div>

                        <div
                          className="content-[''] absolute top-0 left-0 w-full h-full bg-[rgba(0,_0,_0,_0.1)] bg-[radial-gradient(_184.29%_112.9%_at_46.2%_0%,_#ffffff80_0%,_rgba(255,_255,_255,_0)_10%_),_linear-gradient(_90deg,_rgba(255,_255,_255,_0)_0.46%,_rgba(255,_255,_255,_0.2)_0.82%,_rgba(255,_255,_255,_0)_2.18%,_rgba(255,_255,_255,_0)_4.43%,_rgba(0,_0,_0,_0)_4.79%,_rgba(0,_0,_0,_0.06)_5.18%,_rgba(0,_0,_0,_0.15)_5.44%,_rgba(0,_0,_0,_0.2)_5.74%,_rgba(255,_255,_255,_0.2)_5.78%,_rgba(255,_255,_255,_0.17)_6.03%,_rgba(255,_255,_255,_0.09)_6.29%,_rgba(255,_255,_255,_0)_6.56%_)] [box-shadow:-1px_-1px_4px_3px_rgba(0,_0,_0,_0.35)_inset,_1px_1px_4px_0px_rgba(255,_255,_255,_0.35)_inset,_8px_-8px_16px_-8px_rgba(0,_0,_0,_0.08)_inset,_16px_-16px_16px_-16px_rgba(0,_0,_0,_0.25)_inset]"
                          style={{ backgroundBlendMode: 'soft-light' }}
                        >
                          <Image
                            src={product.image || '/placeholder.svg'}
                            alt={product.title}
                            layout="fill"
                            objectFit="cover"
                            className="rounded-xl"
                          />
                        </div>
                      </div>
                    </div>
                  </Link>
                </div>
                <div className="-mt-12 sm:mt-0 flex flex-col gap-5 shrink basis-0 grow">
                  <div
                    aria-hidden="true"
                    className="left-1/2 top-0 w-[300px] center pointer-events-none absolute h-px max-w-full -translate-x-1/2 -translate-y-1/2"
                    style={{
                      background: `linear-gradient(90deg, rgba(0, 0, 0, 0) 0%, rgba(255, 255, 255, 0) 0%, ${product.color} 50%, rgba(0, 0, 0, 0) 100%)`,
                    }}
                  ></div>

                  <h2 className="capitalize text-5xl md:text-4xl font-serif leading-normal">
                    {product.title}
                  </h2>
                  <p className="mt-8">{product.description}</p>
                </div>
              </div>
            </section>
          </CustomSection>
        ))}
      </div>

      {/* Featured Section */}
      <div className="container px-4 md:px-6 py-24">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-serif mb-6">
            Featured Collections
          </h2>
          <p className="text-zinc-400 max-w-2xl mx-auto">
            Explore our most sought-after automotive enhancements, meticulously
            crafted for performance and style.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="bg-zinc-900 rounded-xl p-6 hover:bg-zinc-800 transition-colors">
            <div className="h-48 mb-4 relative overflow-hidden rounded-lg">
              <Image
                src="/placeholder.svg?height=400&width=600"
                alt="Performance Exhaust Systems"
                layout="fill"
                objectFit="cover"
                className="transition-transform duration-500 hover:scale-110"
              />
            </div>
            <h3 className="text-xl font-medium mb-2">
              Performance Exhaust Systems
            </h3>
            <p className="text-zinc-400 mb-4">
              Engineered for optimal flow and distinctive sound profile.
            </p>
            <Link
              href="/exhaust"
              className="text-primary inline-flex items-center"
            >
              Explore <ArrowRight className="ml-2 h-4 w-4" />
            </Link>
          </div>

          <div className="bg-zinc-900 rounded-xl p-6 hover:bg-zinc-800 transition-colors">
            <div className="h-48 mb-4 relative overflow-hidden rounded-lg">
              <Image
                src="/placeholder.svg?height=400&width=600"
                alt="Carbon Fiber Accessories"
                layout="fill"
                objectFit="cover"
                className="transition-transform duration-500 hover:scale-110"
              />
            </div>
            <h3 className="text-xl font-medium mb-2">
              Carbon Fiber Accessories
            </h3>
            <p className="text-zinc-400 mb-4">
              Lightweight components that enhance aesthetics and performance.
            </p>
            <Link
              href="/carbon-fiber"
              className="text-primary inline-flex items-center"
            >
              Explore <ArrowRight className="ml-2 h-4 w-4" />
            </Link>
          </div>

          <div className="bg-zinc-900 rounded-xl p-6 hover:bg-zinc-800 transition-colors">
            <div className="h-48 mb-4 relative overflow-hidden rounded-lg">
              <Image
                src="/placeholder.svg?height=400&width=600"
                alt="Premium Wheel Collection"
                layout="fill"
                objectFit="cover"
                className="transition-transform duration-500 hover:scale-110"
              />
            </div>
            <h3 className="text-xl font-medium mb-2">
              Premium Wheel Collection
            </h3>
            <p className="text-zinc-400 mb-4">
              Precision-engineered wheels for optimal performance and style.
            </p>
            <Link
              href="/wheels"
              className="text-primary inline-flex items-center"
            >
              Explore <ArrowRight className="ml-2 h-4 w-4" />
            </Link>
          </div>
        </div>
      </div>

      {/* Testimonials Section */}
      <div className="bg-zinc-900">
        <div className="container px-4 md:px-6 py-24">
          <h2 className="text-4xl md:text-5xl font-serif text-center mb-16">
            What Our Clients Say
          </h2>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <div className="bg-black bg-opacity-50 p-8 rounded-xl border border-zinc-800">
              <div className="flex items-center mb-4">
                <div className="w-12 h-12 rounded-full bg-zinc-700 mr-4"></div>
                <div>
                  <h4 className="font-medium">James Wilson</h4>
                  <p className="text-zinc-400 text-sm">BMW M3 Owner</p>
                </div>
              </div>
              <p className="text-zinc-300">
                "The performance exhaust system completely transformed my
                driving experience. The sound is incredible and I've noticed
                improved throttle response."
              </p>
            </div>

            <div className="bg-black bg-opacity-50 p-8 rounded-xl border border-zinc-800">
              <div className="flex items-center mb-4">
                <div className="w-12 h-12 rounded-full bg-zinc-700 mr-4"></div>
                <div>
                  <h4 className="font-medium">Sarah Johnson</h4>
                  <p className="text-zinc-400 text-sm">Audi RS5 Owner</p>
                </div>
              </div>
              <p className="text-zinc-300">
                "The carbon fiber interior kit is absolutely stunning. The fit
                and finish is perfect, and it's completely transformed the look
                of my cabin."
              </p>
            </div>

            <div className="bg-black bg-opacity-50 p-8 rounded-xl border border-zinc-800">
              <div className="flex items-center mb-4">
                <div className="w-12 h-12 rounded-full bg-zinc-700 mr-4"></div>
                <div>
                  <h4 className="font-medium">Michael Chen</h4>
                  <p className="text-zinc-400 text-sm">Porsche 911 Owner</p>
                </div>
              </div>
              <p className="text-zinc-300">
                "The customer service was exceptional. They helped me select the
                perfect wheels for my Porsche, and the installation was
                flawless."
              </p>
            </div>
          </div>
        </div>
      </div>

      {/* CTA Section */}
      <div className="border-t border-zinc-800">
        <div className="container px-4 md:px-6 py-24">
          <div className="flex flex-col items-center justify-center space-y-4 text-center">
            <h2 className="text-3xl md:text-4xl font-serif">
              Ready to elevate your automotive experience?
            </h2>
            <p className="text-zinc-400 max-w-[42rem] leading-normal sm:text-lg sm:leading-7">
              Schedule a consultation with our automotive specialists to
              discover the perfect upgrades for your vehicle.
            </p>
            <Button className="mt-4" size="lg">
              Book Consultation
              <ArrowRight className="ml-2 h-4 w-4" />
            </Button>
          </div>
        </div>
      </div>
    </div>
  );
}
